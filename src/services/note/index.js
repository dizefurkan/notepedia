        ...
        handler: (req, res) => {
            ...
            if (title || content) {
                ...
                dbOperations.verifyToken(token).then(result => {
                    ...
                    res.send({ success: true, message: 'Note' + replies.wasCreated });
                }).catch(error => {
                    ...
                    dbOperations.findOne('Note', 'id', req.params.id).then(result => {
                        if (result.data.userId === user.id) {
                            models.Note.update(req.body,
                            {
                                ...
                                res.send({ success: true, message: 'Note' + replies.wasUpdated });
                            })
                        } else {
                ...
                dbOperations.findOne('Note', 'id', req.params.id).then(result => {
                    if (result.data.userId === user.id) {
                        models.Note.destroy({
                            ...
                            res.send({ success: true, message: 'Note' + replies.wasDeleted, result: result });
                        })
                    } else {
                        models.SharedNote.findOne({
                            where: {
                        ...
                        if (result === null) {
                            ...
                            if (result.note.userId === user.id) {
                                models.SharedNote.update(
                                    ...
                                    res.send({ success: true, message: 'Shared Note' + replies.wasUpdated});
                                })
                            } else {
                                res.send({ success: false, message: replies.notFound });
                            }
                    ...
                    if (result === null) {
                        ...
                        if (control.found && control.isHavePermission) {
                            models.SharedNote.destroy(
                                ...
                                res.send({ success: true, message: 'Shared Note' + replies.wasDeleted});
                            });
                        }
